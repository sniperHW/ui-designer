...
4. `装配合同.md` — **UI 风格图、素材替换和运行时装配的 P0 不变量**
...
- **风格图必须服从装配合同**：对既有页面生成风格图、视觉锚点或组件源板前，必须先读取 `装配合同.md`，以用户最后确认的 `.uiw` 锁定尺寸、层级、插槽、状态轮廓和复用关系。风格图只能改变颜色、材质、光影及合同允许的装饰；未通过合同审查的风格图不得用于切图或装配。
- **确认版本必须封版**：用户明确确认页面效果后，必须同步更新页面合同的 `approvedBaseline` 与机器可读 `assemblyGates`，执行合同声明的单一发布门禁命令，并把 UIW、真实预览、门禁、合同、素材和验证脚本纳入版本控制；不得只在聊天或本机未跟踪文件中记录。
