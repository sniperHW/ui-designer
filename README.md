...
- `examples/卡牌.uiw` — 卡牌收集页原型（竖屏 750×1600）：顶部状态栏在公共层（所有页面公用）；底部导航 TabView（商店 / 卡牌 / 战斗 / 城堡 / 成就）只实现卡牌页签，其余留白；牌组为筛选器（牌组 1-5），牌组下方是战力 9888 与各种族兵种数量统计条；下部 TabView（所有卡牌 / 神器 / 宝箱 / 表情）只实现所有卡牌页签，「所有卡牌」用滚动区承载 4×4 卡片可上下滑动。
- `tests/gui-card/` — 上述工程的界面操作自动化（CDP 输入事件驱动真实 UI，不直接写数据）：`node tests/gui-card/build.mjs` 全程界面操作复现搭建（新建工程 → 公共层状态栏 → 底部导航 → 筛选器 → 24 张「游戏卡牌」定制控件实例 → 保存）。卡牌为定制控件「游戏卡牌」，暴露 配型（⛏资源矿 / ⚔战士 / 🏰防御塔）/ 等级 / 进度 / 碎片 / 可升级（⬆）五个属性，实例逐张覆盖；脚本内含「存为定制控件后替换实例固定落在画布根」的对应处理（删除根实例后从控件库实例化回页签内）。
