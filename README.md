...
- **点击交互**：按钮天生可点击，其它任意控件（含定制控件实例）可开启「可点击」；点击效果 = 切换页面 / 返回上一页（回到跳转来路页面，无来路时无效）/ 弹出弹窗；**弹窗在独立弹窗页中设计**（页面列表「弹窗」分组，普通页面禁放弹窗控件），触发时遮罩置顶弹出、点 ✕ / 遮罩 / Esc 关闭；选中控件右键菜单「删除 / 点击」触发效果演示，属性面板也可「▶ 演示」
