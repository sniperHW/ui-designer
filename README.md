...
- **点击交互**：按钮天生可点击，其它任意单个控件（含定制控件实例，实例整体响应）可开启「可点击」标记，弹窗页内控件（如弹窗里的「确定」）同样可配；**定制控件定义内的控件也可配置（定义级，所有实例同步——预览中点击实例上的对应区域即触发）**；点击效果 = 切换页面 / 返回上一页（回到跳转来路页面，无来路时无效）/ 弹出弹窗；**弹窗在独立弹窗页中设计**（页面列表「弹窗」分组；普通页面、公共层与定制控件内禁放弹窗控件，弹窗本体不可删除），触发时遮罩置顶弹出、弹窗内控件可继续触发效果、点 ✕ / 遮罩 / Esc 关闭；选中控件右键菜单「删除 / 点击」触发效果演示，属性面板也可「▶ 演示」
