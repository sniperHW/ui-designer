# ui-designer — 手游 UI 雏形设计工具

线框图 + 交互原型设计工具（Electron）。设计文档见 `概念设计.md`，**当前进度与接手要点见 `开发进度.md`**，当前实现为 M0 + M1 + M2 + 点击交互（M3 首片）。

## 开发

```bash
npm install
npm run dev        # 启动开发模式
npm run typecheck  # TypeScript 检查
npm run build      # 构建产物（out/）
```

## 当前功能（v0.3）

- 内置控件全集：形状（矩形 / 圆角矩形 / 椭圆 / 线段 / 占位图 / 九宫格）、文本、交互（按钮 / 复选框 / 进度条 / 输入框 / 筛选器）、容器（面板 / 弹窗 / 轻提示框 / 滚动区 / 列表 / 网格 / Tab 页签）
- **容器通用机制**：把控件拖到容器内容区自动挂为子控件（几何落点递归判定，支持嵌套）；移动 / 删除 / 复制容器时子控件跟随；子控件裁剪显示，图层树缩进
- **Tab 容器**：默认 3 页签（可增删改名）；页签栏上 / 下、栏高与页签字号可调（属性面板数值或选中后拖页签栏分界手柄）；画布点击页签头切换编辑页
- **定制控件（M2 核心）**：画布组合"存为定制控件"原位替换为实例；以容器为骨架新建；编辑定义全局同步到所有实例；暴露属性（内部子控件属性绑定 + 实例覆盖值）；插槽（Tab 每页签默认开放，面板 / 弹窗 / 滚动区可标记开放，实例可挂内容）；打散实例为普通组合；禁止循环引用；建议锚点（顶部 / 底部拉伸自动吸附）
- **锚点 + 多分辨率预览**：九宫格锚点 × 尺寸模式（固定 / 随父拉伸 / 等比）；状态栏一键切换 16:9 / 18:9 / 19.5:9 / 4:3 只读预览，按锚点规则实时重排；刘海屏安全区参考框
- **筛选器**：标签行单选高亮；项标记 + 绑定列表 / 网格与标记键（预览过滤随 M3 预览实现）
- **点击交互**：按钮天生可点击，其它任意单个控件可开启「可点击」标记，弹窗页内控件（如弹窗里的「确定」）同样可配；**定制控件的可点击标记统一配在定义内控件上（定义级，所有实例同步——预览中点击实例上的对应区域即触发）**，实例被单独选中时属性面板不显示「点击」区、实例自身标记不生效；点击效果 = 切换页面 / 返回上一页（回到跳转来路页面，无来路时无效）/ 弹出弹窗；**弹窗在独立弹窗页中设计**（页面列表「弹窗」分组；普通页面、公共层与定制控件内禁放弹窗控件，弹窗本体不可删除；**弹窗本体的标题栏与弹窗页名双向同步**——重命名弹窗或改标题，列表 / 点击效果下拉 / 标题栏处处一致），触发时遮罩置顶弹出、弹窗内控件可继续触发效果、点 ✕ / 遮罩 / Esc 关闭；选中控件右键菜单「删除 / 点击」触发效果演示，属性面板也可「▶ 演示」
- **轻提示**：任意单个控件可标记「轻提示」（选中控件属性面板「轻提示」区 / 预览中悬停弹出）；**轻提示框在独立轻提示页中设计**（页面列表「轻提示」分组，添加方式与弹窗一致：新建自带居中气泡，尾箭头四向可调，弹出时按空间自动上下翻转）；定制控件的轻提示标记统一配在定义内控件上（定义级，实例悬停对应区域触发），实例属性不显示「轻提示」区；属性面板「▶ 演示轻提示」，Esc / 点击画布关闭
- **原型预览**：工具栏「▶ 预览」全屏运行整套工程——可点击控件跳转 / 返回 / 弹窗（✕ 关闭，弹窗内控件可点）、**标记轻提示的控件悬停弹出轻提示框（鼠标移开关闭）**、Tab 页签就地切换、滚动区滚轮滚动（滑块随动，嵌套时到边界滚动链放行外层）、公共层可交互；Esc 退出
- **启动流程**：打开程序默认未打开工程（欢迎页）；新建（横竖屏/尺寸预设）或打开 `.uiw` 后进入编辑器；未保存修改切换前确认；**欢迎页与「文件」菜单提供最近打开列表**（跨会话持久化，点击不经对话框直达，文件失效自动清理，可一键清除记录）
- **公共层**：工程级共享层，顶部状态栏 / 常驻导航一次编辑、所有页面共享；页面列表顶部"公共层"条目编辑，普通页面中只读，缩略图与导出包含
- 从控件库拖入画布（或点击加到画布中央），选择 / 移动 / 8 向缩放 / 方向键微调
- 多选（Shift / ⌘A）、对齐与分布、图层排序（显隐 / 锁定 / 移层 / 置顶底）
- 页面管理：新建 / 复制 / 删除 / 重命名（✎ 按钮或双击，页面与弹窗一致），缩略图预览
- 网格显示与吸附、滚轮缩放平移、适配窗口
- 撤销 / 重做、复制 / 粘贴 / 再制、删除
- 工程保存 / 打开（`.uiw`，v0.1 为 JSON 明文）、最近打开文件（最多 10 条）、导出当前页 PNG（2x 白底）

## 示例工程

- `examples/卡牌.uiw` — 卡牌收集页原型（竖屏 750×1600）：顶部状态栏在公共层（所有页面公用）；底部导航 TabView（商店 / 卡牌 / 战斗 / 城堡 / 成就）只实现卡牌页签，其余留白；牌组为筛选器（牌组 1-5），牌组下方是战力 9888 与各种族兵种数量统计条；下部 TabView（所有卡牌 / 神器 / 宝箱 / 表情）只实现所有卡牌页签，「所有卡牌」用滚动区承载 4×4 卡片可上下滑动。
- `tests/gui-card/` — 上述工程的界面操作自动化（CDP 输入事件驱动真实 UI，不直接写数据）：`node tests/gui-card/build.mjs` 全程界面操作复现搭建（新建工程 → 公共层状态栏 → 底部导航 → 筛选器 → 24 张「游戏卡牌」定制控件实例 → 保存）。卡牌为定制控件「游戏卡牌」，暴露 配型（⛏资源矿 / ⚔战士 / 🏰防御塔）/ 等级 / 进度 / 碎片 / 可升级（⬆）五个属性，实例逐张覆盖；脚本内含「存为定制控件后替换实例固定落在画布根」的对应处理（删除根实例后从控件库实例化回页签内）。
- `examples/竞技场-界面操作原型.uiw` — 以界面操作方式搭建的同款 4 界面原型（战斗 / 商店 / 卡牌 / 排名，竖屏 750×1600）：顶部状态栏在公共层，底部导航为 TabView；商店「特卖 / 基础」与卡牌「卡组 / 段位 / 图鉴 / 聊天」为嵌套 Tab 内嵌滚动区，排行榜为滚动区。
- `tests/gui-arena/` — 上述原型的界面操作自动化（CDP 输入事件驱动真实 UI，不直接写数据）：`node tests/gui-arena/build.mjs` 全程界面操作复现搭建，`node tests/gui-arena/shot.mjs` 结构校验 + 逐页签截图，`node tests/gui-arena/customize.mjs` / `customize2.mjs` / `customize3.mjs` 为定制控件化改造复现（牌组换筛选器；卡牌 / 特卖礼包 / 榜单条目 / 竞技场宝箱 / 左右侧栏活动做成定制控件并以暴露属性覆盖）。
- `examples/竞技场-4页签原型.uiw` — 参考竞技场手游截图制作的 4 页面原型（战斗 / 商店 / 卡牌 / 排名，默认战斗页），竖屏 750×1600；顶部资源栏与底部页签导航在公共层（含锚点，切分辨率预览可看自适应），各页在自己的页签位叠加高亮。
- `examples/竞技场-TabView原型.uiw` — 同一套界面的方式一实现：单页 + 底部 Tab 容器（页签栏在底部），画布点击页签头即可切换界面，默认选中战斗；导航图标以兄弟节点叠加在页签栏上（页签头只能渲染文字），Tab 容器从顶部资源栏下方开始以免自带白底遮住公共层。
- 两版共享 `examples/arena-lib.mjs`（界面内容与宝箱卡 / 礼包横幅 / 榜单行定制控件定义，暴露属性改文字）；商店页内「特卖 / 基础」为嵌套 Tab 容器（页签栏在底部），页签内容用滚动区承载、可上下滚动（传奇卡包被下边缘裁剪示意继续下滑）；`node examples/build-arena-4tabs.mjs` / `node examples/build-arena-tabview.mjs` 重新生成。

## 用 AI Agent 生成设计工程（skill）

仓库内置了 [skills/uiw-designer/](skills/uiw-designer/)，让 AI agent（ZCode / Claude Code 等）学会直接生成可打开的 `.uiw` 工程：

- `SKILL.md` — 触发条件与生成工作流（读规范 → 生成 → 校验 → 修正循环）
- `references/schema.md` — `ProjectDoc` 文档结构与字段约束（对应 `src/renderer/src/types.ts`）
- `references/widgets.md` — 19 种控件的默认尺寸 / props / 容器语义（对应 `src/renderer/src/widgets/registry.ts`）
- `scripts/validate.mjs` — 无依赖校验器：`node skills/uiw-designer/scripts/validate.mjs <文件.uiw>`，输出带节点路径的错误（✗）/ 警告（⚠），退出码 0/1
- `examples/最小工程.uiw` — 覆盖公共层、定制控件实例 + 暴露属性覆盖、筛选器↔列表联动、底部 Tab 的最小样例

skill 源码放在仓库 `skills/` 目录，**默认未安装**，按需导入到 ZCode：

**方式一：全局 skill（用户级，所有项目可用）**

```bash
# 复制（快照，skill 更新后需重新复制）
mkdir -p ~/.zcode/skills
cp -R skills/uiw-designer ~/.zcode/skills/

# 或软链（随仓库实时更新，推荐本仓库开发者使用）
mkdir -p ~/.zcode/skills
ln -s "$PWD/skills/uiw-designer" ~/.zcode/skills/uiw-designer
```

**方式二：项目级 skill（工作区级，仅本仓库可用，可随 git 分发给团队）**

```bash
mkdir -p .zcode/skills
cp -R skills/uiw-designer .zcode/skills/    # 同样可用软链代替复制
```

放入 `.zcode/skills/` 并提交到 git 后，所有克隆本仓库的人生效。

**生效与验证**：重启会话（或新开会话）后 ZCode 自动发现；在对话里说"帮我生成一个 xxx 界面的 .uiw 设计工程"会自动触发，也可输入 `/uiw-designer` 显式调用。注意同名 skill 的发现顺序为 用户级 `~/.zcode/skills/` → 工作区级 `.zcode/skills/`，先发现者优先（用户级会遮蔽同名工作区级 skill）。若要跨工具共享（Claude Code 等），改放 `~/.agents/skills/`。

### 配套技能（下游管线）

除生成 `.uiw` 的 `uiw-designer` 外，`skills/` 下还有两个把设计工程继续往下游走的配套技能，安装方式同上（复制或软链到 `~/.zcode/skills/` 或 `.zcode/skills/`）：

- [skills/ui-designer-game-ui-h5/](skills/ui-designer-game-ui-h5/) — **`.uiw` → 游戏 UI 美术资产 + H5 原型**。以 `.uiw` 为界面结构与交互的事实来源、参考截图为视觉语言来源，生成去重的美术资产、布局与资产绑定 JSON（`ui-layout.json` + `manifest.json`），并装配为可运行的 H5 原型。适用于「ui-designer 工程转 H5」「按 UI 原型和截图生图」「清理未引用游戏 UI 素材」等任务；不用于单独创建 `.uiw` 工程或只生成孤立 UI 图集。
- [skills/laya-ui-import/](skills/laya-ui-import/) — **`ui-layout.json` → LayaAir 工程**。把 uiw-portable 契约与美术切图导入 LayaAir 工程，生成 ui2 场景 / 预制体 / 数据层 / 交互脚本（`<Widget>.lh`、Binder、Main.ts 等）。与 layaAir 技能配套：本技能负责「契约 → 工程」的导入管线。当提到 `ui-layout.json`、界面契约、切图导入、把原型/H5 设计稿转成 LayaAir 工程时触发。

三者串成完整链路：`uiw-designer` 生成 `.uiw` 设计工程 → `ui-designer-game-ui-h5` 产出资产与 `ui-layout.json` 契约（可直接跑 H5 原型）→ `laya-ui-import` 把契约导入 LayaAir 正式工程。

## 快捷键

⌘Z / ⇧⌘Z 撤销重做 · ⌘C/⌘V/⌘D 复制粘贴再制 · ⌘A 全选 · ⌫ 删除 · 方向键微调（⇧ ×10）· ⌘S 保存 · ⌘O 打开 · ⌘N 新建 · ⌘= / ⌘- 缩放 · Esc 取消选择

## 结构

```
src/main/       Electron 主进程（窗口、文件对话框 IPC）
src/preload/    contextBridge API
src/renderer/   React 界面
  src/store/editorStore.ts   编辑器状态（文档 + 历史 + 视口 + 定制控件）
  src/widgets/registry.ts    控件定义与 SVG 线框渲染（画布/缩略图/导出共用）
  src/widgets/tree.ts        节点树遍历（容器子树/插槽/克隆换 id）
  src/components/            界面组件
```
