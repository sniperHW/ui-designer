...
数据驱动不仅是将 `.uiw` 节点属性搬到 `ui-data.js`。实现列表项、卡牌、商品等 UI 对象前，先联合读取该对象本身、定制控件定义与实例覆盖值，以及它通过源 `clickAction` 关联的详情页、弹窗等 UI 对象，归纳成独立的业务数据对象。涉及重复项或详情关联时，读取 [data-model.md](references/data-model.md) 的建模与绑定契约。

- 以实体为单位归纳字段，例如卡牌的名称、图像、等级、碎片、经验、属性、技能和费用；列表与弹窗用到的字段都进入同一实体或它引用的公共定义。不能只抽取列表字段，再让弹窗从页面常量读取业务值。
- 业务数据使用稳定 `entityId`；列表/牌组/筛选结果保存实体引用，选中状态保存当前实体 ID。布局节点 ID、坐标、CSS、皮肤与数据选择器放在独立的绑定描述中，不充当业务对象模型。`nodeId → props/text` 字典仅可作为派生视图适配层，不能成为业务数据主源。
- 区分公共定义、实例状态和不同业务含义：同种卡牌的皮肤可复用，但不同卡牌的等级、数量等实例值不能共享可变对象；“碎片进度”与“经验进度”等不能因外观相似合并。源信息不足以认定两个视图属于同一实体时，保留独立实例并记录待确认关系。
- 列表项点击时，在执行原始 `clickAction` 的同时传递当前实体上下文；目标页或弹窗读取这份实体数据。不要修改源动作、在视觉覆盖层添加动作、按卡牌下标猜目标数据，或让所有项都显示第一条数据。重复使用同一弹窗布局时，每次打开都重新绑定选中实体。
- 为归纳字段保留来源（源节点、属性、实例覆盖或关联弹窗）。同一字段值冲突时明确记录取值策略；缺失字段保持缺失或使用标明来源的展示默认值，不能补造业务数值或动作。用户提供的权威业务数据优先于示例文案。
- 业务数值只保存一份，展示文本和进度按字段格式化；源文本不能可靠拆分时可暂保留语义命名的原始文本字段，不要丢失单位、精度或特殊状态。资产字段使用 manifest 的 `assetId`，不能把图片路径和布局样式混入实体。
- 验收至少使用两个字段不同的实体：分别打开关联详情，核对对应记录；修改一个实体的共享字段，应同时更新它的列表与详情，另一个实体不变。此验证不能通过新增源工程未定义的按钮或业务动作实现。
...
- `ui-data.js` 放归纳后的实体、公共定义、集合引用与状态；独立的 `data-bindings.json`（或 `ui-layout.json` 中单独的 `dataBindings`）描述 UI 属性如何读取实体字段。`ui.js` 读取布局、数据绑定、渲染器映射和交互上下文；样式只描述布局、缩放与皮肤组合。`ui-layout.json` 中没有的结构不得作为无记录的常量页面拼进去。源 `scroll`、`panel`、`grid`、`list`、`tab`、`dialog` 和 custom widget 必须有不同且可验证的语义映射。
...
交付前逐项确认：`.uiw` 校验通过；布局契约保持节点结构、设计比例与容器类型；每个 `scroll` 经过实际滚动验证；交互索引与 H5 监听器逐项一致；业务实体已联合归纳列表项与关联 UI 的字段，列表与详情读取同一实体，两个不同实体的展示与修改互不串值；每个生成素材有截图来源和消费者；manifest 的 assetId 唯一；没有重复美术资产；无未引用生成散图；H5 可通过 HTTP 服务加载，关键页面、tab、弹窗和跳转可演示；只有 `.uiw` 明确可点击的控件才具有反馈并触发变化。

最终说明应列出输入 `.uiw`、截图、`ui-layout.json`、业务数据与数据绑定、`asset-plan.json`、manifest、审计报告和 H5 入口的位置；简述复用了哪些资产、只新生成了哪些唯一资产，以及清理了哪些未引用输出。
